...
<div class="tt-result-details">\
  <div class="tt-detail-card"><div class="tt-detail-label">Wood Type</div><div class="tt-detail-value">' + esc((r.speciesType || 'Unknown').charAt(0).toUpperCase() + (r.speciesType || '').slice(1)) + '</div><div class="tt-detail-sub">' + esc((r.woodDensity ? r.woodDensity.charAt(0).toUpperCase() + r.woodDensity.slice(1) + ' density' : '') + (r.canopyDescription ? ' · ' + r.canopyDescription : '')) + '</div>' + (r.species ? '<div class="tt-detail-sub" style="margin-top:4px;font-style:italic;">Potential species: ' + esc(r.species) + '</div>' : '') + '</div>\
  <div class="tt-detail-card"><div class="tt-detail-label">Estimated Height</div><div class="tt-detail-value">' + (r.estimatedHeightFt || '?') + ' ft</div><div class="tt-detail-sub">' + esc(r.heightTier || '') + ' tree classification</div><div class="tt-detail-sub" style="margin-top:4px;font-style:italic;">' + (r.heightConfidence === 'low' ? 'Limited reference objects in photo — on-site measurement recommended' : 'Based on visible references in photo — exact height confirmed on-site') + '</div></div>\
